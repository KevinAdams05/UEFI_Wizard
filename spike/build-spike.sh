#!/usr/bin/env bash
#
# build-spike.sh — cross-build uefi_spike (single-file CLI)
#
# Runs on the Haiku Linux cross-build server (kevin@192.168.74.122). Produces
# a single binary at build/uefi_spike — no .hpkg, no resource attachment.
# The spike is meant to be scp'd to the test box and run directly.
#
...
if [ ! -f "$HAIKU_KITS/libbe.so" ]; then
	echo "ERROR: libbe.so not found — build the Haiku tree first" >&2
	exit 1
fi
...
"$CXX" -O0 -g -Wall -Wno-multichar -fno-strict-aliasing \
	"${INCLUDES[@]}" \
	-c "$SPIKE/uefi_spike.cpp" -o "$BUILD/uefi_spike.o"

echo "==> Linking..."
"$CXX" "$BUILD/uefi_spike.o" \
	-B"$HAIKU_DEVEL_LIB" \
	-L"$HAIKU_DEVEL_LIB" \
	-L"$HAIKU_RUNTIME_LIB" \
	-L"$GCC_SYSLIBS_DEVEL" \
